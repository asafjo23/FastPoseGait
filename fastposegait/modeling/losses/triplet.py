...
class TripletLoss(BaseLoss):
    def __init__(self, margin=0.3, loss_term_weight=1.0):
        ...
        self.euclidean_distance = distances.LpDistance(normalize_embeddings=True)
    ...
    def forward(self, embeddings, labels):
        ...
        embeddings = embeddings.view(-1, c)  # Reshape to [p*n, c]

        # Repeat labels for each part
        labels = labels.repeat(p)

        # Mine hard triplets and pairs
        triplet_pairs = self.triplet_miner(embeddings, labels)

        # Compute different losses
        loss_triplet = self.triplet_loss(embeddings, labels, triplet_pairs)
